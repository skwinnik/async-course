...
namespace AccountingService.BackgroundServices {
  public class TaskStatusConsumerBackgroundService : BackgroundService {
        ...
        rabbitContainer.Bus.Advanced.Consume(c => {
          c
            .ForQueue(taskAssignedQueue, handlers =>
              handlers.Add<string>(this.TaskAssignedV1Handler()),
        ...
        rabbitContainer.Bus.Advanced.Consume(c => {
          c
            .ForQueue(taskCompletedQueue, handlers =>
              handlers.Add<string>(this.TaskCompletedV1Handler()),
              config => config.WithConsumerTag("accounting-service"));
        });
    ...
    private IMessageHandler<string> TaskAssignedV1Handler() {
      return async (message, info, cancellationToken) => {
        try {
          ...
          if (!Common.Events.SchemaRegistry.Business_V1_TaskAssigned.TryDeserializeValidated(message.Body, out TaskAssigned result)) {
            ...
            //should be configured for queue
          ...
          await dbContext.TaskAssignedLogs.AddAsync(new Db.Models.TaskAssignedLog { TaskId = result.TaskId, UserId = result.UserId, TimeStamp = result.TimeStamp.ToUniversalTime() });
    ...
    private IMessageHandler<string> TaskCompletedV1Handler() {
      return async (message, info, cancellationToken) => {
        try {
          using var dbContext = await this.dbContextFactory.CreateDbContextAsync(cancellationToken);
